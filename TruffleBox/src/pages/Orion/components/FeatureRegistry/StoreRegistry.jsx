...
const StoreRegistry = () => {
  ...
  useEffect(() => {
    ...
    const fetchConfigOptions = async () => {
      try {
        const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-config`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        });
  ...
  const handleSubmit = async () => {
    const apiEndpoint = `${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/register-store`;
    try {
