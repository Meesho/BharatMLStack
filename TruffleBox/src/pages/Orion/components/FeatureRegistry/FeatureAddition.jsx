...
const FeatureAddition = () => {
  ...
  const fetchFeatureGroups = useCallback(async (entityLabel) => {
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-feature-groups?entityLabel=${entityLabel}`, {
        headers: {
          'Authorization': `Bearer ${user.token}`,
        },
      });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/add-features`, {
        method: 'POST',
        headers: {
