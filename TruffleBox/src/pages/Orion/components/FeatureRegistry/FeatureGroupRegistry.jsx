...
const FeatureGroupRegistry = () => {
  ...
  const fetchEntities = async () => {
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-entites`, {
        headers: {
          'Authorization': `Bearer ${user.token}`,
        },
      });
  ...
  const fetchJobs = useCallback(async () => {
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-jobs?jobType=writer`, {
        headers: {
          'Authorization': `Bearer ${user.token}`,
        },
      });
  ...
  useEffect(() => {
    const fetchFeatureGroupRequests = async () => {
      try {
        const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-feature-group-requests`, {
          headers: {
            'Authorization': `Bearer ${user.token}`,
          },
        });
  ...
  const handleSubmit = async () => {
    ...
    const apiEndpoint = `${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/register-feature-group`;
    try {
