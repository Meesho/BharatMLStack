...
const JobApproval = () => {
  ...
  const fetchJobRequests = useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-job-requests`, {
        headers: {
  ...
  const handleSubmit = async (event, status) => {
    ...
    try {
      ...
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/process-job`, {
        method: 'POST',
        headers: {
