...
const StoreApproval = () => {
  ...
  const fetchStoreRequests = useCallback(async () => {
    try {
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/get-store-requests`, {
        headers: {
          'Authorization': `Bearer ${user.token}`,
        },
      });
      if (!response.ok) throw new Error('Failed to fetch store requests');
  ...
  const handleSubmit = useCallback(async (event, status) => {
    ...
    try {
      ...
      const response = await fetch(`${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/orion/process-store`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) throw new Error(result.error || 'Failed to process store');
