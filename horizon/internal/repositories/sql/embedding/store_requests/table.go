...
const StoreRequestsTableName = "store_requests"
...
func (StoreRequest) TableName() string {
	return StoreRequestsTableName
}

func (StoreRequest) BeforeCreate(tx *gorm.DB) (err error) {
	tx.Statement.SetColumn(constant.CreatedAt, time.Now())
	return
}

func (StoreRequest) BeforeUpdate(tx *gorm.DB) (err error) {
