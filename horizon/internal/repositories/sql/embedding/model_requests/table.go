...
const ModelRequestsTableName = "model_requests"
...
func (ModelRequest) TableName() string {
	return ModelRequestsTableName
}

func (ModelRequest) BeforeCreate(tx *gorm.DB) (err error) {
	tx.Statement.SetColumn(constant.CreatedAt, time.Now())
	return
}

func (ModelRequest) BeforeUpdate(tx *gorm.DB) (err error) {
