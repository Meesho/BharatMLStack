...
func NewMiddleware() Middleware {
	middlewareOnce.Do(func() {
		...
		if err != nil {
			log.Error().Msgf("Error in creating api resolver repository")
		}
...
func (m *MiddlewareHandler) AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if strings.HasPrefix(c.Request.URL.Path, "/login") ||
			...
			strings.HasPrefix(c.Request.URL.Path, "/api/custodian/health") ||
			strings.HasPrefix(c.Request.URL.Path, "/api/1.0/fs-config") ||
			strings.HasPrefix(c.Request.URL.Path, "/api/v1/custodian/predator-config-registry/models/promote") {
			c.Next()
			return
		}
...
func (m *MiddlewareHandler) CheckScreenPermission(c *gin.Context, claims *handler.Claims) {
	...
	if apiResolver.ResolverFn == "" {
		return
	}

	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Unable to resolve API"})
		c.Abort()
		return
	}
	...
	if !exists {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Resolver function not found"})
		c.Abort()
		return
	}
