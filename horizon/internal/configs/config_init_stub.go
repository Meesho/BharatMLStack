...
func bindEnvVars() {
	...
	viper.BindEnv("initial_ingestion_prism_job_id", "INITIAL_INGESTION_PRISM_JOB_ID")
	viper.BindEnv("initial_ingestion_prism_step_id", "INITIAL_INGESTION_PRISM_STEP_ID")
	viper.BindEnv("initial_ingestion_airflow_dag_id", "INITIAL_INGESTION_AIRFLOW_DAG_ID")
	viper.BindEnv("variant_scaleup_prism_job_id", "VARIANT_SCALEUP_PRISM_JOB_ID")
	viper.BindEnv("variant_scaleup_prism_step_id", "VARIANT_SCALEUP_PRISM_STEP_ID")
	viper.BindEnv("variant_scaleup_airflow_dag_id", "VARIANT_SCALEUP_AIRFLOW_DAG_ID")
	viper.BindEnv("variant_onboarding_cron_expression", "VARIANT_ONBOARDING_CRON_EXPRESSION")
	viper.BindEnv("variant_scaleup_cron_expression", "VARIANT_SCALEUP_CRON_EXPRESSION")
	viper.BindEnv("mq_id_topics_mapping", "MQ_ID_TOPICS_MAPPING")
	viper.BindEnv("variants_list", "VARIANTS_LIST")
	viper.BindEnv("skye_scylla_active_config_ids", "SKYE_SCYLLA_ACTIVE_CONFIG_IDS")
	viper.BindEnv("scylla_2_contact_points", "SCYLLA_2_CONTACT_POINTS")
	viper.BindEnv("scylla_2_port", "SCYLLA_2_PORT")
	viper.BindEnv("scylla_2_keyspace", "SCYLLA_2_KEYSPACE")
	viper.BindEnv("scylla_2_username", "SCYLLA_2_USERNAME")
	viper.BindEnv("scylla_2_password", "SCYLLA_2_PASSWORD")
	viper.BindEnv("skye_app_name", "SKYE_APP_NAME")
	viper.BindEnv("horizon_to_skye_scylla_conf_id_map", "HORIZON_TO_SKYE_SCYLLA_CONF_ID_MAP")
	viper.BindEnv("skye_number_of_partitions", "SKYE_NUMBER_OF_PARTITIONS")
	viper.BindEnv("skye_failure_producer_mq_id", "SKYE_FAILURE_PRODUCER_MQ_ID")

	viper.BindEnv("skye_host", "SKYE_HOST")
