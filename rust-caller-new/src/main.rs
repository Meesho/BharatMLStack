...
struct AppState {
    ...
    // Store as Vec<String> wrapped in Arc - protobuf requires String anyway, so avoid conversion
...
async fn retrieve_features(State(state): State<Arc<AppState>>) -> Result<Json<String>, StatusCode> {
    ...
    // Clone Arc pointer (cheap) and use the Vec<String> directly
    let query = retrieve::Query {
        entity_label: state.entity_label.as_ref().to_string(),
        feature_groups: vec![FeatureGroup {
            label: FEATURE_GROUP_LABEL.to_string(), // Use constant to avoid repeated allocation
            feature_labels: state.feature_labels.as_ref().clone(), // Clone Vec<String> (cheap Arc clone)
        }],
        keys_schema: state.keys_schema.as_ref().clone(), // Clone Vec<String> directly
        keys: vec![
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Pre-build static parts once at startup as Vec<String> wrapped in Arc
    // Since protobuf requires String anyway, store as Vec<String> to avoid conversion overhead
