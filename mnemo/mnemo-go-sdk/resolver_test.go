...
func dnsCfg(port int) DNSConfig {
	return DNSConfig{Tenant: "recsys", Store: "catalog", Namespace: "mnemo", DNSZone: "cluster.local", Port: port, Interval: time.Hour}
}
...
func TestDNSResolver_FQDN(t *testing.T) {
	...
	assert.Equal(t, "recsys-catalog-shard-3.mnemo.svc.cluster.local", d.fqdn(3))
}
...
func TestDNSResolver_Refresh_Success(t *testing.T) {
	withLookup(func(_ context.Context, host string) ([]string, error) {
		...
		if host == "recsys-catalog-shard-0.mnemo.svc.cluster.local" {
