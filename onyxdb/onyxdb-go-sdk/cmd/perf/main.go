// perf — load/latency benchmark for the OnyxDB read-server cluster over TCP.
//
...
//   cd onyxdb/onyxdb-go-sdk
//   CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GOWORK=off go build -o /tmp/onyxdb-perf ./cmd/perf
//   gcloud compute scp --tunnel-through-iap /tmp/onyxdb-perf <vm>:~/onyxdb-perf
//
// Run (single vs multi-get):
//   ./onyxdb-perf -etcd 10.138.72.120:2379 -tenant ds -store catalog_geohash_e2e \
...
func main() {
	...
	fmt.Printf("OnyxDB perf — etcd=%s %s/%s keys=%d concurrency=%d batch=%d duration=%s\n",
