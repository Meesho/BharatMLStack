# OnyxDB Control Plane

The control plane manages the full version lifecycle for OnyxDB stores:
...
|----------|---------|-------------|
| `ONYXDB_CP_ADDR` | `:8080` | HTTP listen address |
| `ONYXDB_ETCD_ENDPOINTS` | `localhost:2379` | Comma-separated etcd endpoints |
...
Build from the `onyxdb/` directory (Docker context must include the `schema/` sibling):

```bash
docker buildx build \
  --platform linux/amd64,linux/arm64 \
  -f controlplane/Dockerfile \
  -t ghcr.io/meesho/onyxdb-controlplane:v0.1.0 \
  .
```
