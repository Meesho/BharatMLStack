...
const FeatureGroupApproval = () => {
  ...
  const fetchFeatureGroupRequests = useCallback(async () => {
    try {
      const response = await fetch(
        `${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/online-feature-store/get-feature-group-requests`,
        {
          headers: {
  ...
  const handleSubmit = async (event, status) => {
    ...
    try {
      ...
      const response = await fetch(
        `${URL_CONSTANTS.REACT_APP_HORIZON_BASE_URL}/api/v1/online-feature-store/process-feature-group`,
        {
          method: 'POST',
          headers: {
