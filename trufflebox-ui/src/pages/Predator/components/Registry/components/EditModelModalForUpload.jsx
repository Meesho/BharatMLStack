...
const EditModelModalForUpload = ({ 
  ...
  // Determine if user can upload/edit based on permissions
  const canUploadEdit = hasPermission(service, screenType, ACTIONS.UPLOAD_EDIT);
  ...
  const handleModelNameChange = (modelName) => {
    ...
    if (modelName) {
      ...
      if (selectedModel) {
        // Convert gcs:// to gs:// for GCS path
        let gcsPath = '';
        if (selectedModel.path) {
          gcsPath = selectedModel.path.replace('gcs://', 'gs://');
        }
        
        // Auto-populate metadata and GCS path from selected model
        setFormData(prev => ({
          ...prev,
          gcs_path: gcsPath,
          metadata: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const successMessage = result.data?.message || 'Model updated successfully';
      setSnackbar({
        open: true,
      ...
      >
        ...
        <DialogContent>
          <Alert severity="error">
            You do not have permission to edit models. Please contact your administrator.
          </Alert>
        </DialogContent>
        <DialogActions>
          ...
          <Card variant="outlined">
            <CardContent>
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                  <Autocomplete
                    options={(sourceModels || []).sort((a, b) => (a.name || '').localeCompare(b.name || ''))}
                    getOptionLabel={(option) => option.name || ''}
                    value={sourceModels?.find(model => model.name === formData.metadata.model_name) || null}
                    onChange={(event, newValue) => {
                      handleModelNameChange(newValue?.name || '');
                    }}
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        InputProps={{
                          ...
                          endAdornment: (
                            <>
                              {sourceModelsLoading ? <CircularProgress color="inherit" size={20} /> : null}
...
export default EditModelModalForUpload;
