...
type V1 struct {
	...
	WatchPathEventCallbacks map[string][]func(key, value, eventType string) error
...
func newV1Etcd(config interface{}) Etcd {
	if len(appName) == 0 || len(etcdServers) == 0 {
		log.Panic().Msgf("AppName or EtcdServer is not set")
	}
	...
	v1Etcd := &V1{
		...
		config:                  config,
		appName:                 appName,
		WatchPathEventCallbacks: make(map[string][]func(key, value, eventType string) error),
	}
...
func newV1EtcdFromAppName(config interface{}, appName string) Etcd {
	if len(etcdServers) == 0 {
		log.Panic().Msgf("EtcdServer is not set")
	}
	etcdBasePath := configPath + appName
	servers := strings.Split(etcdServers, ",")
...
func (v *V1) WatchPrefix(ctx context.Context, prefix string, appName string) {
	...
	go func() {
		for watchResp := range watchChan {
			for _, event := range watchResp.Events {
				...
				instances[appName] = v //Update Instance Map for the App Name
				if err != nil {
					log.Error().Err(err).Msgf("unable to parse struct for etcd, not executing watch callbacks")
					continue
				}

				for key, callbacks := range v.WatchPathEventCallbacks {
					watchPath := prefix + key
					if strings.HasPrefix(string(event.Kv.Key), watchPath) {
						for _, callback := range callbacks {
							err = callback(string(event.Kv.Key), string(event.Kv.Value), event.Type.String())
							if err != nil {
								log.Error().Err(err).Msgf("unable to execute event callback for path %s", key)
							}
						}
					}
				}
	...
	for key, data := range *dataMap {
		...
		if v.isTerminalPathParameter(mKey) && valueType.Kind() == reflect.Struct {
			...
			case reflect.Int:
				...
				val.SetMapIndex(reflect.ValueOf(mapKey), reflect.ValueOf(int(intVal)))
			case reflect.Int64:
				intVal, err := strconv.ParseInt(data, 10, 64)
				if err != nil {
					return err
...
// IsLeafNodeExist checks if a node exists at the given path
func (v *V1) IsLeafNodeExist(path string) (bool, error) {
...
// RegisterWatchPathCallbackWithEvent registers an event-aware callback function to be called when a change is detected in the given path
func (v *V1) RegisterWatchPathCallbackWithEvent(path string, callback func(key, value, eventType string) error) error {
	_, ok := v.WatchPathEventCallbacks[path]
	if ok {
		v.WatchPathEventCallbacks[path] = append(v.WatchPathEventCallbacks[path], callback)
	} else {
		v.WatchPathEventCallbacks[path] = []func(key, value, eventType string) error{callback}
	}
	return nil
}
