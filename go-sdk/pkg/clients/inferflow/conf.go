...
func validConfigs(configs *ClientConfig) (bool, error) {
	if configs.Host == "" {
		return false, fmt.Errorf("inferflow service host is invalid, configured value: %v", configs.Host)
	}
	if configs.Port == "" {
		return false, fmt.Errorf("inferflow service port is invalid, configured value: %v", configs.Port)
	}
	if configs.DeadlineExceedMS <= 0 {
		return false, fmt.Errorf("inferflow service deadline exceed timeout is invalid, configured value: %v",
