...
const (
	...
	p2PCacheGlobalCacheTTLInSecondsSuffix = "_GLOBAL_CACHE_TTL_IN_SECONDS"
	p2PCacheClusterNameSuffix             = "_CLUSTER_NAME"
...
func BuildP2PCacheConfFromEnv(envPrefix string) (*P2PCacheConf, error) {
	if !viper.IsSet(envPrefix+p2PCacheEnabledSuffix) ||
		...
		!viper.IsSet(envPrefix+p2PCacheGlobalCacheTTLInSecondsSuffix) ||
		!viper.IsSet(envPrefix+p2PCacheClusterNameSuffix) ||
		...
		return nil, fmt.Errorf("failed to load p2p cache. invalid p2p cache configs, keys: %s %s %s %s %s %s %s %s",
			envPrefix+p2PCacheEnabledSuffix,
			envPrefix+p2PCacheOwnPartitionSizeInBytesSuffix,
			envPrefix+p2PCacheGlobalSizeInBytesSuffix,
			envPrefix+p2PCacheGlobalCacheTTLInSecondsSuffix,
			envPrefix+p2PCacheClusterNameSuffix,
	...
	globalCacheTTLInSeconds := viper.GetInt(envPrefix + p2PCacheGlobalCacheTTLInSecondsSuffix)
	clusterName := viper.GetString(envPrefix + p2PCacheClusterNameSuffix)
	...
	return &P2PCacheConf{
		...
		GlobalCacheTTLInSeconds: globalCacheTTLInSeconds,
