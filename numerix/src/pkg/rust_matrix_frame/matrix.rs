...
type BinaryOp<V> = fn(
    &Vector<<V as VectorOps>::Scalar>,
    &Vector<<V as VectorOps>::Scalar>,
) -> Vector<<V as VectorOps>::Scalar>;
type BinaryOpResult<V> = fn(
    &Vector<<V as VectorOps>::Scalar>,
    &Vector<<V as VectorOps>::Scalar>,
) -> Result<Vector<<V as VectorOps>::Scalar>, Mat2DError>;
type UnaryOp<V> = fn(&Vector<<V as VectorOps>::Scalar>) -> Vector<<V as VectorOps>::Scalar>;
type UnaryOpResult<V> =
    fn(&Vector<<V as VectorOps>::Scalar>) -> Result<Vector<<V as VectorOps>::Scalar>, Mat2DError>;
...
impl<V: VectorOps> Operation<V> {
    ...
    fn get_unary_ops() -> HashMap<&'static str, Operation<V>> {
        ...
        m.insert(
            "percentile_rank",
            Operation::UnaryResult(V::percentile_rank),
        );
        m.insert(
            "norm_percentile_0_99",
            Operation::UnaryResult(V::norm_percentile_0_99),
        );
        m.insert(
            "norm_percentile_5_95",
            Operation::UnaryResult(V::norm_percentile_5_95),
        );
        m
    }
}
    ...
    V::Scalar: Copy + PartialOrd + Default,
{
    ...
    ) -> Result<Self, Mat2DError> {
        if rows * cols != matrix.len() {
            return Err(Mat2DError::MatrixSizeMismatch(
                matrix.len(),
                rows * cols,
                rows,
                cols,
    ...
    pub fn calculate(
        &self,
        expr: &str,
        meta: HashMap<String, Vector<V::Scalar>>,
    ) -> Result<Vector<V::Scalar>, Mat2DError>
    where
        ...
        for token in tokens {
            match (
                ...
                (Some(op), _, _, _) => {
                    let right = stack
                        .pop()
                        .ok_or_else(|| Mat2DError::StackUnderflow(token.to_string()))?;
                    let left = stack
                        .pop()
                        .ok_or_else(|| Mat2DError::StackUnderflow(token.to_string()))?;
