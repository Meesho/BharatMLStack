...
pub async fn watch_etcd_path(
    ...
    tokio::spawn(async move {
        loop {
            let mut client = client_arc.lock().await;
            let options = WatchOptions::new().with_prefix().with_progress_notify();
            let watch_result = client
                .watch(etcd_absolute_path.clone(), Some(options))
                .await;
                            ...
                            match ev.event_type() {
                                EventType::Put => {
                                    if let Some(kv) = ev.kv() {
                                        if let Ok(json) =
                                            serde_json::from_slice::<Value>(kv.value())
                                        {
                                            if let Some(expression) =
                                                json.get("expression").and_then(|e| e.as_str())
                                            {
                                                ...
                                                let key_str =
                                                    key_str.split("/").last().unwrap().to_string();
