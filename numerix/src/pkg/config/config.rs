...
fn init_config() -> AppConfig{
    ...
    match builder.set_default(TELEGRAF_UDP_HOST, TELEGRAF_UDP_HOST_VALUE) {
        Ok(b) => builder = b,
        Err(e) => {
            logger::fatal("Failed to set default value for telegraf_udp_host to 'localhost'", Some(&e))
        }
    }
    ...
    match builder.set_default(TELEGRAF_UDP_PORT, TELEGRAF_UDP_PORT_VALUE) {
        Ok(b) => builder = b,
        Err(e) => {
            logger::fatal("Failed to set default value for telegraf_udp_port to 8125", Some(&e))
        }
    }
    ...
    match builder.set_default(LOG_SAMPLING_RATE, LOG_SAMPLING_RATE_VALUE) {
        Ok(b) => builder = b,
        Err(e) => {
            logger::fatal("Failed to set default value for log_sampling_rate to 1.0", Some(&e))
        }
    }
    ...
    let settings = match builder.add_source(Environment::default()).build() {
        Ok(cfg) => cfg,
        Err(e) => {
            logger::fatal("Failed to build configuration from environment variables", Some(&e))
        }
    };
    ...
    match settings.try_deserialize() {
        Ok(cfg) => cfg,
        Err(e) => {
            logger::fatal("Failed to deserialize configuration into AppConfig struct", Some(&e))
        }
    }
}
