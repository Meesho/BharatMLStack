...
pub async fn init_server() {
    ...
    let listener = match listener {
        ...
        Err(e) => {
            logger::fatal(format!("Failed to bind TCP listener to address {}", address), Some(&e))
        }
    };
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let _ = metrics::count("numerix.channel.grpc.queue_capacity", grpc_max_capacity as u64, &[]);
    ...
    let grpc_rx_stream = tokio_stream::wrappers::ReceiverStream::new(grpc_rx)
    ...
    logger::info(format!("Numerix service is running at port {}", configs.application_port));
    loop {
        let accept_result = listener.accept().await;
        let (stream, addr) = match accept_result {
            Ok((stream, addr)) => (stream, addr),
            Err(e) => {
                logger::error(format!("Failed to accept incoming TCP connection on {}", address), Some(&e));
                continue;
            }
        };
                ...
                Ok(bytes_read) if bytes_read >= 24 => {
                    if &buff[0..24] == b"PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n" {
                        if let Err(e) = grpc_sender.send(stream).await {
                            logger::error(format!("Failed to send gRPC stream from {} to server", addr), Some(&e));
                        }
                    } else {
                        if let Err(e) = http_sender.send(stream).await {
                            logger::error(format!("Failed to send HTTP stream from {} to server", addr), Some(&e));
                        }
                    }
                },
                Ok(bytes_read) => {
                    logger::error(format!("Insufficient protocol data from {}: got {} bytes, need 24", addr, bytes_read), None);
                },
                Err(e) => {
                    logger::error(format!("Failed to peek connection data from {}", addr), Some(&e));
                }
            }
        });
    }
