...
pub async fn init_config() {
    if let Err(_) = EXPRESSION_CONFIG_ETCD_PATH.set("/expression-config".to_string()) {
        logger::fatal("Failed to set EXPRESSION_CONFIG_ETCD_PATH, it was already initialized", None)
    }
    
    let etcd_path = match EXPRESSION_CONFIG_ETCD_PATH.get() {
        Some(path) => path,
        None => {
            logger::fatal("EXPRESSION_CONFIG_ETCD_PATH was not set and could not be initialized", None)
        }
    };
    ...
    if EXPRESSION_META_MAP.set(expression_meta_map_arc.clone()).is_err() {
    ...
    etcd::watch_etcd_path(etcd_path, expression_map_for_watch, expression_meta_map_for_watch).await;
}
...
pub fn get_exression(key: &str) -> String {
    ...
    let expression_map = match expression_map {
        ...
        None => {
            logger::error(format!("Expression map not initialized when looking up compute_id: {}", key), None);
            return "".to_string();
        }
    };
    ...
    let expression_map = match expression_map {
        ...
        Err(e) => {
            logger::error(format!("Failed to read expression map lock for compute_id: {}", key), Some(&e));
            return "".to_string();
        }
    };
    ...
    match expression_map.get(key) {
        ...
        None => {
            logger::error(format!("Expression not found for compute_id: {}", key), None);
            "".to_string()
        }
    }
}
...
pub fn get_meta_data(compute_id: &str) -> Vec<String> {
    ...
    let expression_meta_map = match expression_meta_map {
        ...
        None => {
            logger::error(format!("Expression meta map not initialized when looking up compute_id: {}", compute_id), None);
            return Vec::new();
        }
    };  
    ...
    let expression_meta_map = match expression_meta_map {
        ...
        Err(e) => {
            logger::error(format!("Failed to read expression meta map lock for compute_id: {}", compute_id), Some(&e));
            return Vec::new();
        }
    };      
    match expression_meta_map.get(compute_id) {
        ...
        None => {
            logger::error(format!("Meta data not found for compute_id: {}", compute_id), None);
            return Vec::new();
        }
    }
}
