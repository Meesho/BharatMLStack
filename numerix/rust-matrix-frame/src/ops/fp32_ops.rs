...
impl VectorOps for F32Ops {
    ...
    fn percentile_rank(vec: &Vector<Self::Scalar>) -> Result<Vector<Self::Scalar>, Mat2DError> {
        ...
        for i in 0..vec.len() {
            result[i] = i as f32 / (vec.len() - 1) as f32;
...
impl F32Ops {
    fn calculate_min_max_norm(vec: &Vector<f32>, min: f32, max: f32) -> Vector<f32> {
    ...
    fn compute_percentile(values: &[f32], p: f32) -> f32 {
        if p == 0.0 {
            return values[0];
        }
        let index = p / 100.0 * (values.len() - 1) as f32;
        let floor = index.floor();
        if floor == (values.len() - 1) as f32 {
