...
// buildEntitiesFromPairs creates the entity/feature structures for pairwise.
// Each pair expands into a row with first target features (prefixed "first_"),
// second target features (prefixed "second_"), and pair-level features.
func buildEntitiesFromPairs(
	targets []*pb.Target,
	pairs []*pb.TargetPair,
	contextFeatures []*pb.ContextFeature,
	targetSchema []*pb.FeatureSchema,
	pairSchema []*pb.FeatureSchema,
) ([]string, [][]string, map[string][]string) {

	numPairs := len(pairs)

	// Entity dimension: pair index as the ID
	entities := []string{"pair_id"}
	pairIds := make([]string, numPairs)
	for i := range pairIds {
		pairIds[i] = strconv.Itoa(i)
	}
	entityIds := [][]string{pairIds}

	features := make(map[string][]string)

	// First and second target IDs as features
	firstIds := make([]string, numPairs)
	secondIds := make([]string, numPairs)
	for i, pair := range pairs {
		firstIds[i] = targets[pair.FirstTargetIndex].Id
		secondIds[i] = targets[pair.SecondTargetIndex].Id
	}
	features["first_target_id"] = firstIds
	features["second_target_id"] = secondIds

	// Expand target features with "first_" and "second_" prefixes
	for schemaIdx, schema := range targetSchema {
		firstVals := make([]string, numPairs)
		secondVals := make([]string, numPairs)
		for i, pair := range pairs {
			firstTarget := targets[pair.FirstTargetIndex]
			secondTarget := targets[pair.SecondTargetIndex]
			if schemaIdx < len(firstTarget.FeatureValues) {
				firstVals[i] = string(firstTarget.FeatureValues[schemaIdx])
			}
			if schemaIdx < len(secondTarget.FeatureValues) {
				secondVals[i] = string(secondTarget.FeatureValues[schemaIdx])
			}
		}
		features["first_"+schema.Name] = firstVals
		features["second_"+schema.Name] = secondVals
	}

	// Pair-level features
	for schemaIdx, schema := range pairSchema {
		vals := make([]string, numPairs)
		for i, pair := range pairs {
			if schemaIdx < len(pair.FeatureValues) {
				vals[i] = string(pair.FeatureValues[schemaIdx])
			}
		}
		features[schema.Name] = vals
	}
