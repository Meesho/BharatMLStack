...
// Parameter named `selected`, not `metrics`, for the same shadowing reason as in
// runtime_metrics.go — inferflow's metric package is `pkg/metrics`.
func WithRuntimeMetrics(selected ...RuntimeMetric) Option {
	return func(c *config) { c.runtimeMetrics = append(c.runtimeMetrics, selected...) }
}
