...
def _fetch_schema_with_retry(url: str, max_retries: int = _MAX_RETRIES) -> dict:
    ...
    for attempt in range(max_retries):
        try:
            ...
            last_exception = SchemaFetchError(
                f"URL Error connecting to inference service: {e.reason}"
            )
...
def get_feature_schema(
    ...
    if api_path is None:
        api_path = os.getenv(
            "INFERENCE_PATH", "/api/v1/inference/mp-config-registry/get_feature_schema"
        )
    ...
    if not features:
        raise SchemaNotFoundError(
            f"No features found in schema for model_config_id={model_config_id}, version={version}"
        )
    ...
    while reader.has_more():
        ...
        if field_number == 1 and wire_type == 2:  # user_id
            ...
            if len(metadata_bytes) > 0:
                ...
                result.compression_enabled, result.version, result.format_type = (
                    unpack_metadata_byte(result.metadata_byte)
                )
