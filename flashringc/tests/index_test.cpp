...
static void test_put_get() {
    ...
    uint32_t slot = idx.put(h, /*mem_id=*/1, /*offset=*/100, /*length=*/42);
    assert(idx.size() == 1);
    ...
    assert(lr.length == 42);
    assert(lr.last_access == 0);  // before the update that just happened
    (void)slot;
}
...
static void test_put_update() {
    ...
    idx.put(h, 1, 100, 42);
    idx.put(h, 2, 200, 84);  // update same key
    assert(idx.size() == 1);
...
static void test_evict_oldest() {
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        hashes.push_back(h);
        idx.put(h, static_cast<uint32_t>(i), static_cast<uint32_t>(i * 100), 10);
    }
    assert(idx.size() == 8);
...
static void test_ring_wrap_auto_evict() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        idx.put(h, static_cast<uint32_t>(i), static_cast<uint32_t>(i), 1);
    }
    assert(idx.size() == 4);
...
static void test_many_inserts() {
    ...
    for (uint32_t i = 0; i < N; ++i) {
        ...
        idx.put(h, i, i, static_cast<uint16_t>(i & 0xFFFF));
    }
    assert(idx.size() == N);
