...
func (o *OrderRetrieveHandler) Retrieve(userId string, startTimestampMs int64, endTimestampMs int64, limit int32) ([]model.FlattenedOrderEvent, error) {
	...
	orderedWeeks := o.buildOrderedWeeks(startTimestampMs, endTimestampMs)
	tablesToFields := o.buildTableToFieldsMapping(orderedWeeks)
	...
	weekToEvents, err := o.deserializeWeeks(weekToData, userId)
	if err != nil {
		return nil, err
	}

	return o.mergeFilterAndLimit(orderedWeeks, weekToEvents, startTimestampMs, endTimestampMs, int(limit)), nil
}

func (o *OrderRetrieveHandler) buildOrderedWeeks(startTimestampMs int64, endTimestampMs int64) []int {
...
func (o *OrderRetrieveHandler) buildTableToFieldsMapping(orderedWeeks []int) map[string][]string {
	tablesToFields := make(map[string][]string)
	for _, week := range orderedWeeks {
		tableName := getOrderTableName(week / constants.WeeksPerBucket)
...
func (o *OrderRetrieveHandler) fetchDataInParallel(tablesToFields map[string][]string, userId string) (map[string][]byte, error) {
	...
	for tableName, fields := range tablesToFields {
		...
		g.Go(func() (err error) {
			defer func() {
				if r := recover(); r != nil {
					log.Error().Msgf("panic occurred while fetching order data from table %s: %v", tblName, r)
					err = fmt.Errorf("panic during data fetch from table %s: %v", tblName, r)
				}
			}()
			data, fetchErr := o.scyllaDb.RetrieveInteractions(tblName, userId, cols)
			if fetchErr != nil {
...
// work is done in parallel to reduce latency. For a single week the sequential
// path avoids goroutine/channel overhead.
func (o *OrderRetrieveHandler) deserializeWeeks(weekToData map[string][]byte, userId string) (map[string][]model.FlattenedOrderEvent, error) {
	weekToEvents := make(map[string][]model.FlattenedOrderEvent, len(weekToData))
	...
	if len(work) == 1 {
		w := work[0]
		ddb, err := blocks.DeserializePSDB(w.data, enum.InteractionTypeOrder)
		if err != nil {
		...
		events, ok := eventsRaw.([]model.FlattenedOrderEvent)
		if !ok {
...
func (o *OrderRetrieveHandler) mergeFilterAndLimit(orderedWeeks []int, weekToEvents map[string][]model.FlattenedOrderEvent, startTimestampMs int64, endTimestampMs int64, limit int) []model.FlattenedOrderEvent {
	result := make([]model.FlattenedOrderEvent, 0, limit)
