...
func (o *OrderRetrieveHandler) Retrieve(userId string, startTimestampMs int64, endTimestampMs int64, limit int32) ([]model.FlattenedOrderEvent, error) {
	...
	for _, ddb := range weekToDeserializedBlocks {
		...
		allEvents = append(allEvents, events.([]model.FlattenedOrderEvent)...)
	}
	...
	filteredEvents := make([]model.FlattenedOrderEvent, 0)
...
func (o *OrderRetrieveHandler) buildTableToFieldsMapping(startTimestampMs int64, endTimestampMs int64, getTableNameFunc func(int) string) map[string][]string {
