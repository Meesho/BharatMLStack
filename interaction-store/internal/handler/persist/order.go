...
func (p *OrderPersistHandler) Persist(userId string, data any) error {
	orderEvents, ok := data.([]model.FlatOrderEvent)
	if !ok {
		return fmt.Errorf("unexpected data type for persistence: got %T, want []FlatOrderEvent", data)
	}

	bucketEvents, weekFlags := p.partitionEventsByBucket(orderEvents)
...
func (p *OrderPersistHandler) partitionEventsByBucket(events []model.FlatOrderEvent) (map[int][]model.FlatOrderEvent, []bool) {
	bucketEvents := make(map[int][]model.FlatOrderEvent)
...
func (p *OrderPersistHandler) persistToBucket(bucketIdx int, userId string, events []model.FlatOrderEvent, weekFlags []bool) error {
...
func (p *OrderPersistHandler) deserializeExistingData(data map[string]interface{}) (map[string]*blocks.DeserializedPSDB, error) {
	...
	for column, value := range data {
		ddb, err := blocks.DeserializePSDB(value.([]byte), enum.InteractionTypeOrder)
		if err != nil {
...
func (p *OrderPersistHandler) mergeEvents(
	newEvents []model.FlatOrderEvent,
	storageBlocks map[string]*blocks.DeserializedPSDB,
) (map[string][]model.FlatOrderEvent, []string, []string, error) {
	finalEvents := make(map[string][]model.FlatOrderEvent)
...
func (p *OrderPersistHandler) getExistingOrderEvents(ddb *blocks.DeserializedPSDB, column string) ([]model.FlatOrderEvent, error) {
	data, err := ddb.RetrieveEventData()
	if err != nil {
		return nil, fmt.Errorf("retrieve event data failed for column %s: %w", column, err)
	}
	events, ok := data.([]model.FlatOrderEvent)
	if !ok {
...
func (p *OrderPersistHandler) mergeAndTrimEvents(existing []model.FlatOrderEvent, newEvent model.FlatOrderEvent) []model.FlatOrderEvent {
	if len(existing) > 0 {
		lastTimestamp := existing[len(existing)-1].OrderedAt
		if utils.TimestampDiffInWeeks(newEvent.OrderedAt, lastTimestamp) >= 24 {
			existing = existing[:0]
		}
	}
...
func (p *OrderPersistHandler) persistEvents(
	bucketIdx int,
	userId string,
	finalEvents map[string][]model.FlatOrderEvent,
...
func (p *OrderPersistHandler) updateMetadata(userId string, metadataToUpdate, metadataToInsert map[string]interface{}) {
	for column, count := range metadataToUpdate {
		_ = p.scyllaDb.UpdateMetadata(getOrderMetadataTableName(), userId, column, count)
	}

	if len(metadataToInsert) > 0 {
		_ = p.scyllaDb.PersistMetadata(getOrderMetadataTableName(), userId, metadataToInsert)
	}
}
...
func (p *OrderPersistHandler) buildAndSerializePermanentStorageDataBlock(data []model.FlatOrderEvent) ([]byte, error) {
