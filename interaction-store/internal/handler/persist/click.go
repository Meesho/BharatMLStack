...
func (p *ClickPersistHandler) updateMetadata(userId string, metadataToUpdate map[string]interface{}) {
	...
	if len(metadataToUpdate) > 0 {
		if err := p.scyllaDb.UpdateMetadata(tableName, userId, metadataToUpdate); err != nil {
			log.Error().Msg(fmt.Sprintf("failed to update click metadata for userId=%s, table=%s, columns=%v: %v", userId, tableName, metadataToUpdate, err))
		}
	}
}
